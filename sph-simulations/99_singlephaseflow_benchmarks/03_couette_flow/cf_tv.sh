#!/bin/bash
#SBATCH --job-name=cf_tv              # Job name
...
#SBATCH --output=cf_tv_%j.log         # Path to the standard output and error files relative to the working directory
#SBATCH --error=cf_tv_%j.err          # Path to the standard error file
...
/usr/local.nfs/software/openmpi/5.0.1_gcc-12.2_cuda-12.3/bin/mpirun -np 2 python3 run_couette_flow_TV.py ${NL} "${INIT}"
popd > /dev/null

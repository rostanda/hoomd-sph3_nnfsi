...
_particle_data = dict(
    ...
    typeid=dict(
        np_type=np.int32, value=[0, 0, 0, 1, 1], new_value=[1, 1, 1, 0, 0], shape=(Np,)
    ),
    mass=dict(
        np_type=np.floating,
        value=[5, 4, 3, 2, 1],
    ...
    image=dict(
        ...
        new_value=np.linspace(-20, 10, Np * 3, dtype=np.int32).reshape(Np, 3),
        shape=(Np, 3),
    ),
...
_bond_data = dict(
    _N=Nb,
    typeid=dict(
        np_type=np.unsignedinteger, value=[0, 1], new_value=[1, 0], shape=(Nb,)
    ),
    group=dict(
...
@pytest.fixture(
    scope="function",
    params=[
        (section_name, prop_name, prop_dict)
...
@pytest.fixture(
    scope="function", params=["", "ghost_", "_with_ghost"], ids=lambda x: x.strip("_")
)
def affix(request):
...
@pytest.fixture(
    scope="function", params=[check_type, check_shape, check_getting, check_setting]
)
...
class TestLocalSnapshots:
    ...
    @skip_mpi4py
    @pytest.mark.cupy_optional
    def test_cpu_global_properties(
        self, base_simulation, base_snapshot, global_property
    ):
    ...
    @pytest.mark.cupy_optional
    def test_arrays_properties(
        self, base_simulation, section_name_dict, affix, property_check
    ):

...
def update_positions(snap):
    if snap.communicator.rank == 0:
        ...
        snap.particles.position[:] += rs.multivariate_normal(mean, cov, size=shape[:-1])
    return snap
        ...
        for attr in dir(hoomd_snap):
            ...
            for prop in dir(getattr(hoomd_snap, attr)):
                if prop[0] == "_":
                    ...
                    assert getattr(getattr(gsd_snap, attr), prop) == getattr(
                        getattr(hoomd_snap, attr), prop
                    )
                else:
...
def random_inds(n):
    return np.random.choice(np.arange(n), size=int(n * np.random.rand()), replace=False)
...
def test_tps(simulation_factory, two_particle_snapshot_factory):
    ...
    sim.operations.writers.append(
        hoomd.write.CustomWriter(action=list_writer, trigger=hoomd.trigger.Periodic(1))
    )
...
_state_args = [((10, ["A"]), 10), ((5, ["A", "B"]), 20), ((8, ["A", "B", "C"]), 4)]
...
def test_state_from_gsd(
    ...
    for step in range(1, nsteps):
        ...
        set_types(snap, random_inds(snap_params[0]), snap_params[1], particle_type)
...
@skip_gsd
def test_state_from_gsd_box_dims(
    device, simulation_factory, lattice_snapshot_factory, tmp_path
):
...
@skip_gsd
def test_state_from_gsd_frame(
    simulation_factory, lattice_snapshot_factory, device, state_args, tmp_path
):
    snap_params, nsteps = state_args
...
def test_writer_order_initial(simulation_factory, two_particle_snapshot_factory):
...
def test_operations_setting(tmp_path, simulation_factory, lattice_snapshot_factory):
    ...
    operations += hoomd.update.BoxResize(
        ...
        box=hoomd.variant.box.Interpolate(
            hoomd.Box.cube(10), hoomd.Box.cube(20), hoomd.variant.Ramp(0, 1, 0, 100)
        ),
    )
    ...
    new_operations += hoomd.update.BoxResize(
        trigger=80,
        box=hoomd.variant.box.Interpolate(
            hoomd.Box.cube(300), hoomd.Box.cube(20), hoomd.variant.Ramp(0, 1, 0, 100)
        ),
    )
    new_operations += hoomd.write.GSD(filename=tmp_path / "bar.gsd", trigger=20)
    new_operations += hoomd.write.Table(20, logger=hoomd.logging.Logger(["scalar"]))
...
def test_logging():
    logging_check(
        ...
        (),
        {
            "final_timestep": {"category": LoggerCategories.scalar, "default": True},
            "seed": {"category": LoggerCategories.scalar, "default": True},
            "timestep": {"category": LoggerCategories.scalar, "default": True},
            "tps": {"category": LoggerCategories.scalar, "default": True},
            "walltime": {"category": LoggerCategories.scalar, "default": True},
        },
    )

...
template<class CL> void celllist_dimension_test(std::shared_ptr<ExecutionConfiguration> exec_conf)
    ...
    std::shared_ptr<ParticleData> pdata_3 = sysdef_3->getParticleData();
        ...
        h_pos.data[1].y = h_pos.data[1].z = 0.0;
        h_pos.data[2].x = Scalar(2.0);
        h_pos.data[2].y = h_pos.data[2].z = 0.0;
        }
...
UP_TEST(CellList_dimension)
    {
    celllist_dimension_test<CellList>(std::shared_ptr<ExecutionConfiguration>(
...
UP_TEST(CellList_adj)
    {
    celllist_adj_test<CellList>(std::shared_ptr<ExecutionConfiguration>(
        new ExecutionConfiguration(ExecutionConfiguration::CPU)));
    }
...
UP_TEST(CellList_small)
    {
    celllist_small_test<CellList>(std::shared_ptr<ExecutionConfiguration>(
        new ExecutionConfiguration(ExecutionConfiguration::CPU)));
    }
...
UP_TEST(CellListGPU_small)
    {
    celllist_small_test<CellListGPU>(std::shared_ptr<ExecutionConfiguration>(
...
template<class CL> void celllist_large_test(std::shared_ptr<ExecutionConfiguration> exec_conf)
    {
...
UP_TEST(CellList_large)
    {
    celllist_large_test<CellList>(std::shared_ptr<ExecutionConfiguration>(
        new ExecutionConfiguration(ExecutionConfiguration::CPU)));
    }
...
UP_TEST(CellListGPU_large)
    {
    celllist_large_test<CellListGPU>(std::shared_ptr<ExecutionConfiguration>(
        new ExecutionConfiguration(ExecutionConfiguration::GPU)));
    }
#endif

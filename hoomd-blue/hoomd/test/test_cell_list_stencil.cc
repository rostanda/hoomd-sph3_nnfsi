...
void celllist_stencil_basic_test(std::shared_ptr<ExecutionConfiguration> exec_conf)
    ...
    // start with a simple simulation box size 3
    std::shared_ptr<SystemDefinition> sysdef_3(
        new SystemDefinition(1, BoxDim(3.0), 2 ,0, exec_conf, nullptr, false));
    std::shared_ptr<ParticleData> pdata = sysdef_3->getParticleData();
    ...
    cls->compute(31);
        {
        ArrayHandle<Scalar4> h_stencil(cls->getStencils(),
                                       access_location::host,
                                       access_mode::read);
        ...
        for (unsigned int cur_stencil = 0; cur_stencil < 27; ++cur_stencil)
            ...
            if (i != 0)
                cell_dist2 += Scalar(0.25) * (abs(i) - 1) * (abs(i) - 1);
            if (j != 0)
                cell_dist2 += Scalar(0.25) * (abs(j) - 1) * (abs(j) - 1);
            if (k != 0)
                cell_dist2 += Scalar(0.25) * (abs(k) - 1) * (abs(k) - 1);
...
UP_TEST(CellListStencil_cpu)
    {
    celllist_stencil_basic_test<CellList>(std::shared_ptr<ExecutionConfiguration>(
...
UP_TEST(CellListStencil_gpu)
    {
    celllist_stencil_basic_test<CellListGPU>(std::shared_ptr<ExecutionConfiguration>(
